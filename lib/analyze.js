...
exports.convertDgmlToDot = convertDgmlToDot;
 ...
 */
function findQLPackPath(codeqlPath, language, logger) {
    ...
    if (!fs.existsSync(qlpacksPath)) {
        baseMsg(`Directory structure in ${codeqlPath}: ${fs.readdirSync(codeqlPath).join(", ")}`);
        throw new Error(`QLPacks directory does not exist: ${qlpacksPath}`);
    }
    // Output contents of the qlpacks directory for debugging
    baseMsg(`QLPacks directory contents: ${fs.readdirSync(qlpacksPath).join(", ")}`);
    // Check for codeql directory
    const codeqlPackPath = path.join(qlpacksPath, "codeql");
    if (!fs.existsSync(codeqlPath)) {
        baseMsg(`Directory structure in qlpacks: ${fs.readdirSync(qlpacksPath).join(", ")}`);
    ...
    baseMsg(`CodeQL pack directory contents: ${fs.readdirSync(codeqlPath).join(", ")}`);
    ...
    try {
        ...
        throw new Error(`Failed to find ${language}-queries version directory: ${util.getErrorMessage(error)}`);
    }
}
async function generateConfigQueries(codeqlPath, dbPath, logger) {
    const tempDir = path.join(dbPath, "temp");
    if (!fs.existsSync(tempDir)) {
        // log error and exit:
        logger.error(`Temporary directory ${tempDir} does not exist. Please ensure the database path is correct.`);
        throw new Error(`Temporary directory ${tempDir} does not exist. Please ensure the database path is correct.`);
    }
 ...
 * ```
 */
async function convertDgmlToDot(dgmlFile, dotFile, logger) {
    ...
    try {
        ...
        if (!root)
            throw new Error("Missing <DirectedGraph> root element");
        ...
        for (const edge of linksSection) {
            ...
            if (!src || !dst) {
                logger.info(`Skipping link without Source/Target: ${JSON.stringify(raw)}`);
                continue;
            }
...
async function runQueries(sarifFolder, memoryFlag, addSnippetsFlag, threadsFlag, cleanupLevel, diffRangePackDir, automationDetailsId, config, logger, features) {
    ...
    for (const language of config.languages) {
        try {
            ...
            await generateConfigQueries(path.parse(config.codeQLCmd).dir, databasePath, logger);
            ...
            const javaQlpackPath = findQLPackPath(path.parse(config.codeQLCmd).dir, "java", logger);
            await codeql.databaseInterpretResultsCustom(databasePath, path.join(javaQlpackPath, "experimental/quantum/PrintCBOMGraph.ql"), dgmlDir, "dgml", config.debugMode ? "-vv" : "-v");
            const cbomDotFile = path.join(sarifFolder, "cbom.dot");
            const cbomDgmlFile = path.join(dgmlDir, `${language}`, "print-cbom-graph.dgml");
