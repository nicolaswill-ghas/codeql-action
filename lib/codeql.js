...
function setCodeQL(partialCodeql) {
    cachedCodeQL = {
        ...
        databaseInterpretResultsCustom: resolveFunction(partialCodeql, "databaseInterpretResultsCustom"),
...
async function getCodeQLForCmd(cmd, checkVersion) {
    const codeql = {
        ...
        async databaseInitCluster(config, sourceRoot, processName, qlconfigFile, overlayDatabaseMode, logger) {
            ...
            // log the location
            logger.info(`Code-scanning config generated at: ${codeScanningConfigFile}`);
            // read & log the contents
            const fileContents = await fs.promises.readFile(codeScanningConfigFile, "utf8");
        ...
        async databaseInterpretResultsCustom(databasePath, queryPath, outputFile, outputFormat, verbosityFlag = "-v") {
            const codeqlArgs = [
                "database",
                "interpret-results",
                `--format=${outputFormat}`,
